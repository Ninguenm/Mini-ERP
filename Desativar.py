import os
import tkinter as tk
import tkinter.ttk as ttk
import pygubu
import CadastrarDesativacao,CadastrarDesativacaoDes

PROJECT_PATH = os.path.abspath(os.path.dirname(__file__))
PROJECT_UI = os.path.join(PROJECT_PATH, "Admin.ui")

class AdminApp:
    def __init__(self, master=None):
        # build ui
        self.toplevel1 = tk.Toplevel()
        self.frame2 = tk.Frame(self.toplevel1)
        self.toplevel1.title("Desativar Equipamento")
        self.label2 = tk.Label(self.frame2)
        self.label2.configure(font='{arial} 16 {}', justify='left', relief='groove', takefocus=True)
        self.label2.configure(text='Desativar')
        self.label2.place(anchor='nw', relx='0.23', rely='0.04', width='250', x='0', y='0')
        
        self.button2 = tk.Button(self.frame2)
        self.button2.configure(font='{Arial} 11 {}', text='Menu')
        self.button2.place(anchor='nw', height='30', relx='0.39', rely='0.7', width='100', x='0', y='0')
        self.button2.configure(command=self.fecha)
        
        self.button1 = tk.Button(self.frame2)
        self.button1.configure(font='{Arial} 11 {}', text='Equipamento')
        self.button1.place(anchor='nw', height='40', relx='0.048', rely='0.34', width='170', x='0', y='0')
        self.button1.configure(command=CadastrarDesativacao.abre)
        
        self.button3 = tk.Button(self.frame2)
        self.button3.configure(font='{Arial} 11 {}', text='Equipamento em Des.')
        self.button3.place(anchor='nw', height='40', relx='0.58', rely='0.34', width='170', x='0', y='0')
        self.button3.configure(command=CadastrarDesativacaoDes.abre)
        
        self.frame2.configure(background='#a5d6ef', borderwidth='5', height='200', relief='sunken')
        self.frame2.configure(width='450')
        self.frame2.place(anchor='nw', relx='0.003', rely='0.003', x='0', y='0')
        self.toplevel1.configure(background='#669bdb', borderwidth='5', height='215', relief='raised')
        self.toplevel1.configure(width='465')

        app_width =465
        app_height =215

        screen_width =self.toplevel1.winfo_screenwidth()
        screen_height=self.toplevel1.winfo_screenheight()

        x = (screen_width/2)-(app_width/2)
        y = (screen_height/2)-(app_height/2)

        self.toplevel1.geometry(f'{app_width}x{app_height}+{int(x)}+{int(y)}')


        # Main widget
        self.mainwindow = self.toplevel1
    

    def run(self):
        self.mainwindow.mainloop()

    def fecha(self):
        self.mainwindow.destroy()


def abre():
    app = AdminApp()
    app.run()

#abre()
